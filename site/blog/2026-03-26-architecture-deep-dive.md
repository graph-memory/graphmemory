...
```mermaid
graph TD
    Clients["MCP Clients<br/>(Claude Code, Cursor, etc.)"]
    Clients -->|Streamable HTTP| Server

    subgraph Server["HTTP Server"]
        MCP["MCP Sessions"]
        REST["REST API /api/*"]
        WS["WebSocket /api/ws"]
        Queue["PromiseQueue (per project)<br/>Serializes all mutations"]
    ...
    subgraph Storage["Graphology (DirectedGraph)"]
        Nodes["Nodes + edges + embeddings in memory"]
    end

    Managers --> Storage

    Mirror["File Mirror<br/>.notes/ .tasks/ .skills/"]
    Embedder["Embedder<br/>BGE-M3 (ONNX)"]
    TreeSitter["tree-sitter<br/>WASM parser<br/>TS/JS/TSX/JSX"]
