...
users:
  alice:
    ...
    passwordHash: "$scrypt$16384$8$1$salt$hash"   # generated by: mcp-graph-memory users add
...
| `passwordHash` | Scrypt hash for UI password login (generate with `mcp-graph-memory users add`) |
