...
Next time someone (or an AI) asks "how do I add a new API endpoint?", `recall_skills` finds this recipe and returns the exact steps.
...
- "To refactor a class, first check cross_references for all usages"
...
### `search_skills` — precise lookup

Default `minScore: 0.5`. Best for: "do we have a skill for X?"

### `recall_skills` — broad discovery
