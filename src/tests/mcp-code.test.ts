...
describe('MCP code tools', () => {
  ...
  describe('list_files', () => {
    it('returns 2 files sorted with correct symbolCounts', async () => {
      const files = json<FileEntry[]>(await call('list_files'));
    ...
    it('filter "graph" returns 1 file', async () => {
      const files = json<FileEntry[]>(await call('list_files', { filter: 'graph' }));
      expect(files.length).toBe(1);
      expect(files[0].fileId).toBe('src/graph.ts');
    });

    it('filter "src/" returns all 2 files', async () => {
      const files = json<FileEntry[]>(await call('list_files', { filter: 'src/' }));
      expect(files.length).toBe(2);
    });

    it('filter "nonexistent" returns empty', async () => {
      const files = json<FileEntry[]>(await call('list_files', { filter: 'nonexistent' }));
      expect(files.length).toBe(0);
    });

    it('limit=1 returns first alphabetically', async () => {
      const files = json<FileEntry[]>(await call('list_files', { limit: 1 }));
      expect(files.length).toBe(1);
      expect(files[0].fileId).toBe('src/graph.ts');
    });
  ...
  describe('get_file_symbols', () => {
    describe('src/graph.ts', () => {
      it('returns 4 symbols sorted by startLine', async () => {
        const syms = json<SymEntry[]>(await call('get_file_symbols', { fileId: 'src/graph.ts' }));
    ...
    describe('src/search.ts', () => {
      it('returns 2 symbols with search function', async () => {
        const syms = json<SymEntry[]>(await call('get_file_symbols', { fileId: 'src/search.ts' }));
  ...
  describe('search_code', () => {
    describe('basic scoring', () => {
      it('top hit is updateFile with score 1.0', async () => {
        const hits = json<CodeHit[]>(await call('search_code', { query: 'update file', topK: 1, bfsDepth: 0, searchMode: 'vector' }));
      ...
      it('results sorted by score desc', async () => {
        const hits = json<CodeHit[]>(await call('search_code', { query: 'update file', topK: 1, bfsDepth: 0 }));
    ...
    describe('BFS via imports edge', () => {
      it('search function is top hit at depth=1', async () => {
        const hits = json<CodeHit[]>(await call('search_code', { query: 'search function', topK: 1, bfsDepth: 1, searchMode: 'vector' }));
        const ids = hits.map(h => h.id);
      ...
      it('depth=2 reaches src/graph.ts via imports', async () => {
        const hits = json<CodeHit[]>(await call('search_code', { query: 'search function', topK: 1, bfsDepth: 2, minScore: 0 }));
        const ids = hits.map(h => h.id);
        expect(ids).toContain('src/graph.ts');
      });
    });
    ...
    describe('maxResults', () => {
      it('maxResults=1 returns exactly 1 result', async () => {
        const hits = json<CodeHit[]>(await call('search_code', { query: 'update file', topK: 5, bfsDepth: 2, maxResults: 1, searchMode: 'vector' }));
        expect(hits.length).toBe(1);
        expect(hits[0].id).toBe('src/graph.ts::updateFile');
      });
    });
    ...
    describe('bfsDecay', () => {
      it('bfsDecay=1.0 keeps seed score for BFS nodes', async () => {
        const hits = json<CodeHit[]>(await call('search_code', { query: 'update file', topK: 1, bfsDepth: 1, bfsDecay: 1.0, minScore: 0.99, searchMode: 'vector' }));
        expect(hits.some(h => h.id === 'src/graph.ts')).toBe(true);
      });

      it('bfsDecay=0.0 filters BFS nodes to score 0', async () => {
        const hits = json<CodeHit[]>(await call('search_code', { query: 'update file', topK: 1, bfsDepth: 1, bfsDecay: 0.0, minScore: 0.01, searchMode: 'vector' }));
        expect(hits.length).toBe(1);
        expect(hits[0].id).toBe('src/graph.ts::updateFile');
      });
  ...
  describe('get_symbol', () => {
    describe('full content', () => {
      it('updateFile has all expected fields', async () => {
        const sym = json<SymbolResult>(await call('get_symbol', { nodeId: 'src/graph.ts::updateFile' }));
      ...
      it('file node has kind=file and isExported=false', async () => {
        const sym = json<SymbolResult>(await call('get_symbol', { nodeId: 'src/graph.ts' }));
        expect(sym.kind).toBe('file');
        expect(sym.isExported).toBe(false);
      });

      it('DocGraph type has correct attributes', async () => {
        const sym = json<SymbolResult>(await call('get_symbol', { nodeId: 'src/graph.ts::DocGraph' }));
    ...
    describe('search function', () => {
      it('docComment mentions BFS', async () => {
        const sym = json<SymbolResult>(await call('get_symbol', { nodeId: 'src/search.ts::search' }));
  ...
  // =========================================================================
  // search_files (code file search)
  // =========================================================================

  describe('search_files', () => {
    it('top hit is src/graph.ts with score 1.0 and correct symbolCount', async () => {
      const hits = json<CodeFileHit[]>(await call('search_files', { query: 'graph code file' }));
    ...
    it('search code file query hits src/search.ts', async () => {
      const hits = json<CodeFileHit[]>(await call('search_files', { query: 'search code file' }));
      expect(hits[0].fileId).toBe('src/search.ts');
      expect(hits[0].symbolCount).toBe(2);
    });

    it('unknown query returns empty', async () => {
      const hits = json<CodeFileHit[]>(await call('search_files', { query: 'xyzzy unknown', minScore: 0.1 }));
      expect(hits.length).toBe(0);
    });

    it('topK=1 returns at most 1 result', async () => {
      const hits = json<CodeFileHit[]>(await call('search_files', { query: 'graph code file', topK: 1 }));
      expect(hits.length).toBe(1);
    });

    it('minScore=0.9 returns only exact match', async () => {
      const hits = json<CodeFileHit[]>(await call('search_files', { query: 'graph code file', minScore: 0.9 }));
      expect(hits.length).toBe(1);
      expect(hits[0].fileId).toBe('src/graph.ts');
    });
  });
