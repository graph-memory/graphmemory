...
|------|---------|-------------|
| `code_list_files` | List all indexed source files | Get an overview of the codebase |
| `code_get_file_symbols` | List symbols in a file | Explore a file's exports and structure |
| `code_search` | Semantic search across code symbols | Find functions/classes by what they do |
| `code_get_symbol` | Get full details of a symbol | Read signature, JSDoc, body, line numbers |
| `code_search_files` | Search at file level | Find which files are relevant to a topic |
...
Use `code_search` with a description of what you're looking for:
...
Use `code_get_symbol` with the symbol ID from search results. This returns the full picture: signature, JSDoc comment, body text, file location, line numbers, export status.

### 3. Understand file structure

Use `code_get_file_symbols` to see all symbols in a file — helpful for understanding module organization. The result includes `isExported` flag to distinguish public API from internal helpers.

### 4. Discover relevant files

Use `code_search_files` for a quick file-level search before diving into symbols. Lighter than `code_search` when you just need to identify which files to explore.

### 5. Cross-reference with docs

Use `docs_cross_references` (requires both `graphs.docs.include` and `graphs.code.include`) to find everywhere a symbol appears — in both code definitions and documentation examples. See the dedicated guide for details.
...
- `code_search` works best with descriptive queries about **what the code does**, not exact symbol names
- For exact name lookup, use `code_get_symbol` directly with the node ID
- `code_get_file_symbols` shows `isExported` — useful for distinguishing public API from internals
- `code_search_files` is useful as a first step when exploring unfamiliar codebases
- The `docs_cross_references` tool (see dedicated guide) is the most comprehensive way to understand a symbol
- BFS expansion in `code_search` follows `imports`, `contains`, and `extends` edges — set `bfsDepth: 2` to discover related modules
- Code graph edges track `imports` for relative imports and tsconfig path aliases (`@/lib/foo`); external packages are skipped
- BFS in `code_search` excludes reverse `imports` edges — popular utility files won't flood results
