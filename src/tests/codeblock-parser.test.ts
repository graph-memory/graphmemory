...
`;
  ...
  it('extracts interface from TS', async () => {
    expect(await extractSymbols(tsCode, 'typescript')).toContain('TokenPayload');
  });

  it('extracts function from TS', async () => {
    expect(await extractSymbols(tsCode, 'typescript')).toContain('createToken');
  });

  it('extracts class from TS', async () => {
    expect(await extractSymbols(tsCode, 'typescript')).toContain('AuthService');
  });

  it('extracts type alias from TS', async () => {
    expect(await extractSymbols(tsCode, 'typescript')).toContain('Role');
  });

  it('extracts enum from TS', async () => {
    expect(await extractSymbols(tsCode, 'typescript')).toContain('Status');
  });

  it('extracts const variable from TS', async () => {
    expect(await extractSymbols(tsCode, 'typescript')).toContain('SECRET');
  });

  it('extracts arrow function from TS', async () => {
    expect(await extractSymbols(tsCode, 'typescript')).toContain('handler');
  });

  it('extracts correct count from TS', async () => {
    expect(await extractSymbols(tsCode, 'typescript')).toHaveLength(7);
  });
...
`;
    ...
    it('extracts function', async () => {
      expect(await extractSymbols(jsCode, 'javascript')).toContain('query');
    });

    it('extracts const', async () => {
      expect(await extractSymbols(jsCode, 'javascript')).toContain('pool');
    });
  });

  describe('TSX/JSX variants', () => {
    it('tsx language accepted', async () => {
      expect(await extractSymbols('const x = 1;', 'tsx')).toContain('x');
    });

    it('jsx language accepted', async () => {
      expect(await extractSymbols('const x = 1;', 'jsx')).toContain('x');
    });

    it('ts language accepted', async () => {
      expect(await extractSymbols('const x = 1;', 'ts')).toContain('x');
    });

    it('js language accepted', async () => {
      expect(await extractSymbols('const x = 1;', 'js')).toContain('x');
    });
  });

  describe('non-TS/JS languages', () => {
    it('yaml returns empty', async () => {
      expect(await extractSymbols('key: value', 'yaml')).toHaveLength(0);
    });

    it('python returns empty', async () => {
      expect(await extractSymbols('def foo(): pass', 'python')).toHaveLength(0);
    });

    it('empty language returns empty', async () => {
      expect(await extractSymbols('const x = 1;', '')).toHaveLength(0);
    });
  });

  it('malformed code returns empty', async () => {
    const badCode = `
function {{{ this is not valid
  syntax at all !!!
`;
    expect(await extractSymbols(badCode, 'typescript')).toHaveLength(0);
  });

  it('empty code returns empty', async () => {
    expect(await extractSymbols('', 'typescript')).toHaveLength(0);
  });
});
...
describe('parseFile code block extraction', () => {
  ...
  let chunks: Awaited<ReturnType<typeof parseFile>>;
  let textChunks: typeof chunks;
  let codeChunks: typeof chunks;
  let allChildChunks: typeof chunks;

  beforeAll(async () => {
    chunks = await parseFile(fixtureContent, fixturePath, projectDir, 4);
    textChunks = chunks.filter(c => c.language === undefined);
    codeChunks = chunks.filter(c => c.language !== undefined);
    allChildChunks = chunks.filter(c => c.id.includes('::code-'));
  });
  ...
  describe('Authentication TS code blocks', () => {
    let authTsChunks: typeof chunks;

    beforeAll(() => {
      const tsChunks = chunks.filter(c => c.language === 'typescript');
      authTsChunks = tsChunks.filter(
  ...
  describe('JS code block symbols', () => {
    let jsChunks: typeof chunks;

    beforeAll(() => {
      jsChunks = chunks.filter(c => c.language === 'javascript');
    });
  ...
  describe('API Client TS block', () => {
    let apiClientChunk: typeof chunks[number] | undefined;

    beforeAll(() => {
      const tsChunks = chunks.filter(c => c.language === 'typescript');
      apiClientChunk = tsChunks.find(c => c.id.includes('API Client'));
    });
